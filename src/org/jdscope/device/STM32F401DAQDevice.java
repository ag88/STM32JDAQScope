...
public class STM32F401DAQDevice extends Device {
	...
	public void setup() {
		super.setup();
		params.add(new ParamWordSize(0));
	...
	public boolean connect(String portName, int portRate) throws IOException, InterruptedException {
		...
		bexpect(getName().concat(" ready"), timeout);
	...
	public void run() throws IOException, InterruptedException {
		...
		int timeout = 5000;
		wordsize = ((ParamWordSize) params.get(0)).getValue();
		int count = ((ParamCount) params.get(1)).getValue();
		...
		for (Param<?> p : params) {
			...
			if (!p.ischanged())
				continue;
			if (p instanceof ParamWordSize) {
				wordsize = (int) p.getValue();
				sendCommand(CMD_SETWSIZE,wordsize,3);
				expect("Setting word size to:", timeout);
			} else if (p instanceof ParamTrigger) {
				...
				sendCommand(CMD_SETWAIT, count, 3);
		...
		*/
		...
		ByteBuffer buf = readSample(length);
		buf.rewind();
		WordConv wconv = new WordConv(wordsize);
		IntBuffer ints = wconv.fromWordbuf(buf, BUFSIZE, BUFSIZE - count);
		ints.rewind();
		while(ints.hasRemaining()) {
			...
			case 12:
				b = ints.get() & 0xFFF;
				v =	(double) b * VMAX / 4095.0;
				break;
			case 8:
			default:
				b = ints.get() & 0xFF;
				v =	(double) b * VMAX / 255.0;					
			}
			data.add(v);
		}		 		
		databuf.swap();
